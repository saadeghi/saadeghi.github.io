...
<Head
  title={`${data.project.title} – ${data.resume.title}`}
...
<div class="flex flex-col gap-12 lg:gap-20">
  <div>
    <a href="/projects/" class="inline-flex gap-4 items-center">
      ...
      >
        <path
          d="M12.0224 0.0022583L0.0246582 12L12.0224 23.9978L14.5336 21.4308L6.8885 13.7857H80V10.2143H6.8885L14.5336 2.56922L12.0224 0.0022583Z"
          fill="currentColor"
        />
      </svg>
      Back
    ...
    <div class="flex flex-col gap-4 text-center lg:text-left">
      ...
      <div class="flex flex-col items-center gap-4 lg:flex-row">
        ...
        {#if data.project.url && data.project.date}
          <span class="hidden text-black/50 lg:inline">–</span>
        {/if}
        {#if data.project.date}
          <span class="text-black/50">{formatDate(data.project.date)}</span>
        {/if}
      </div>
    </div>
  </div>

  <div class="[&_a]:border-b [&_a]:border-b-black">
  ...
  {#if data.project.tags && data.project.tags.length}
    <div class="flex flex-wrap gap-2">
      {#each data.project.tags as tag}
        <span
          class="rounded-3xl border border-black/10 px-3 py-1 text-black/40"
        >
          {tag}
        </span>
      {/each}
